...
//import com.madgag.wordle.PlayAnalysis.{BrucieBonusAndIncompleteKnowledge, FResult, IncompleteKnowledge, augmentWithRemainingCachedBetaThresholds}
...
//  )
//
//  def augmentWithRemainingCachedBetaThresholds(
//    incompleteKnowledges: Seq[IncompleteKnowledge]
//  ): Seq[BrucieBonusAndIncompleteKnowledge] = {
//    incompleteKnowledges.sorted(IncompleteKnowledge.OptimalOrdering).foldRight((0, List.empty[BrucieBonusAndIncompleteKnowledge])) {
//      case (incompleteKnowledge, (accBrucieBonus: Int, augmentedList: List[BrucieBonusAndIncompleteKnowledge])) =>
//        (
...
//          cachedFResultsFor(params).fold(Left(IncompleteKnowledge(params, None))) { result =>
//            result.map(_.guessSum).left.map(threshold => IncompleteKnowledge(params, Some(threshold)))
//          }
//      }.separate
//
//      val incompleteKnowledgesSorted: Seq[IncompleteKnowledge] =
//        incompleteKnowledges.toSeq.sorted(IncompleteKnowledge.OptimalOrdering)
//
...
//  }.filter(_.candidatesPartition.possibleCandidates.sizeIs == 1).sortBy(_.candidatesPartition.evennessScore)
//
