...
class PlayFirstPossibleWordTest extends AnyFlatSpec with Matchers {
  ...
  it should "sadly fail on some words" in {
    val failedWords = corpus.commonWords.filter( targetWord =>
      !Game(targetWord).playWith(player).isSuccess
    )
    ...
    println(Game(exampleOfFailingWord).playWith(player).text)
  }
}
